...
class EngadgetViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
	
	var articleJson = [Article]()
	
	//IBOutlets
	@IBOutlet weak var EngadgetTableView: UITableView!
	
	//ViewDidLoad
	override func viewDidLoad() {
		super.viewDidLoad()
		EngadgetTableView.delegate = self
		EngadgetTableView.dataSource = self
		
		fetchData(closure: {arrayArticles in
			self.articleJson = arrayArticles!
			self.EngadgetTableView.reloadData()
		})
	}
	...
	func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		let cell = tableView.dequeueReusableCell(withIdentifier: "EngadgetCell", for: indexPath) as! EngadgetTableViewCell
	...
	func fetchData(closure: @escaping ([Article]?) -> ()) {
		...
		let urlString = "https://newsapi.org/v1/articles?source=engadget&sortBy=top&apiKey=a368d3c0637848d0a275cf2a83cfcbfe"
